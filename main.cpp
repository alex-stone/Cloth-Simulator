...
// Used with Menu
const char** clothFiles;
const char** sceneFiles;
float* dampValues;
float* toleranceValues;
float* springValues;
...
static int parameters_sub_id;
static int damping_sub_id;
static int tolerance_sub_id;
static int spring_constant_sub_id;
...
static int dampingBase;
static int numDampValues;

static int toleranceBase;
static int numToleranceValues;

static int springConstantBase;
static int numSpringValues;
...
void initTestFiles() {
    ...
    clothFiles = new const char*[numCloths];
    sceneFiles = new const char*[numScenes];

    clothFiles[0] = "test/freefall.test";
    clothFiles[1] = "test/freefallDetail.test";
    clothFiles[2] = "test/flag.test";
    clothFiles[3] = "test/horizCloth.test";
    clothFiles[4] = "test/twoCorner.test";
    clothFiles[5] = "test/oneCorner.test";

    sceneFiles[0] = "shapes/empty.test";
    sceneFiles[1] = "shapes/centerSphere.test";
    sceneFiles[2] = "shapes/4spheres.test";
    sceneFiles[3] = "shapes/2spheres.test";
    sceneFiles[4] = "shapes/plane.test";
    sceneFiles[5] = "shapes/cube.test";

    dampValues = new float[3];
    toleranceValues = new float[3];
    springValues = new float[3];
...
void drawClothPoints() {
    ...
    float radius = scene->cloth->getPointDrawSize();

    if(spherePoints) {

         for(int h = 0; h < scene->cloth->getHeight(); h++) {

            for(int w = 0; w < scene->cloth->getWidth(); w++) {
                Vertex* vert = scene->cloth->getVertex(w, h);
...
void drawStretchSprings() {
    ...
    vector<Spring*> temp = scene->cloth->getStretchSprings();
...
void drawShearSprings() {
    vector<Spring*> temp = scene->cloth->getShearSprings();
...
void drawBendSprings() {
    vector<Spring*> temp = scene->cloth->getBendSprings();
...
void renderCloth() {
    ...
    scene->cloth->updateNormals();

    if(debugFunc) {
        cout << "Successfully Update Normals" << endl;
    }
 
    for(int h = 0; h < scene->cloth->getHeight()-1; h++) {
        ...
        for(int w = 0; w < scene->cloth->getWidth(); w++) {
            ...
            Vertex* temp = scene->cloth->getVertex(w, h);
            ...
            float s = (float) w / (float) (scene->cloth->getWidth()-1);
            float t = (float) h / (float) (scene->cloth->getHeight()-1);
            glTexCoord2d(s,t);
            ...
            // Vertex 2:
            temp = scene->cloth->getVertex(w, h+1);

            // Vertex 2's Normal values
            glNormal3f(temp->getNorm().x, temp->getNorm().y, temp->getNorm().z);

            // Texture Coordinates, only t value changes.
            t = (float) (h+1) / (float) (scene->cloth->getHeight()-1);
...
void preUpdateCalculation() {
    ...
    if(gravity) {
        scene->cloth->addConstantAccel(gravityAccel);
    }

    if(wind) {
        scene->cloth->updateNormals(); // Only update Normals if NEEDED
        scene->cloth->addTriangleForce(windForce);
    }
...
void makeDrawLists() {
    for(int i = 0; i < scene->getNumShapes(); i++) {
        shapeDrawLists[i] = drawShape(scene->getShape(i));
    }
}
...
void menu(int num) {
    ...
    // Pauses the Simulation when you right click
   // running = false;
    ...
    // Change Texture:
    if(num >= textureBase && num < (textureBase + numTextures)) {
        currentTex = num - textureBase;
        glBindTexture(GL_TEXTURE_2D, textures[currentTex]);
        currentTexture = textureNames[currentTex];

    }

    // Change Cloth
    if(num >= clothBase && num < (clothBase + numCloths)) {
        inputFile = clothFiles[num-clothBase];

        scene->resetScene(inputFile, shapeFile, euler, useFloor);
        frameNum = 0;

    }

    // Change Scene
    if(num >= sceneBase && num < (sceneBase + numScenes)) {
        shapeFile = sceneFiles[num - sceneBase];

        scene->resetScene(inputFile, shapeFile, euler, useFloor);
        frameNum = 0;

        // Initialize Shape Draw Lists
        shapeDrawLists = new GLuint[scene->getNumShapes()]; 
        makeDrawLists();

    }

    // Change Integration Method: 
    if(num >= integrationBase && num < (integrationBase + numIntegrationMethods)) {
        if(num - integrationBase == 0) {
            euler = true;
        } else {
            euler = false;
        }
    ...
    if(num >= dampingBase && num < (dampingBase + numDampValues)) {

        running = false;

        scene->changeDamp(dampValues[num - dampingBase]);
    }

    if(num >= toleranceBase && num < (toleranceBase + numToleranceValues)) {

        running = false;

        scene->changeTolerance(dampValues[num - toleranceBase]);
    }

    if(num >= springConstantBase && num < (springConstantBase+ numSpringValues)) {

        running = false;

        scene->changeSpringConstant(dampValues[num - springConstantBase]);
    }
...
void createMenu() {
    ...
    numDampValues = 3;
    numSpringValues = 3;
    numToleranceValues = 3;

    textureBase = 2;
    clothBase = textureBase + numTextures;
    sceneBase = clothBase + numCloths;
    integrationBase = sceneBase + numScenes;
    dampingBase = integrationBase + numIntegrationMethods;
    toleranceBase = dampingBase + numDampValues;
    springConstantBase = toleranceBase + numToleranceValues;
    ...
    glutAddMenuEntry("Free Fall 20x20", clothBase);
    glutAddMenuEntry("Free Fall 50x50", clothBase+1);
    glutAddMenuEntry("Flag", clothBase+2);
    glutAddMenuEntry("4 Corners", clothBase+3);
    glutAddMenuEntry("2 Corners", clothBase+4);
    glutAddMenuEntry("1 Corner", clothBase+5);
    ...
    glutAddMenuEntry("2 Balls", sceneBase+3);
    glutAddMenuEntry("Plane/Tabletop", sceneBase+4);
    glutAddMenuEntry("Cube", sceneBase+5);
    ...
    damping_sub_id = glutCreateMenu(menu);
    glutAddMenuEntry("Low  (0.01)", dampingBase);
    glutAddMenuEntry("Med  (0.015)", dampingBase+1);
    glutAddMenuEntry("High (0.02)", dampingBase+2);

    tolerance_sub_id = glutCreateMenu(menu);
    glutAddMenuEntry("Low  (0.01)", toleranceBase);
    glutAddMenuEntry("Med  (0.05)", toleranceBase+1);
    glutAddMenuEntry("High (0.1)", toleranceBase+2);

    spring_constant_sub_id = glutCreateMenu(menu);
    glutAddMenuEntry("Low  (10)", springConstantBase);
    glutAddMenuEntry("Med  (50)", springConstantBase+1);
    glutAddMenuEntry("High (100)", springConstantBase+2);

    parameters_sub_id = glutCreateMenu(menu);
    glutAddSubMenu("Damping Factor", damping_sub_id);
    glutAddSubMenu("Tolerance", tolerance_sub_id);
    glutAddSubMenu("Spring Constant", spring_constant_sub_id);
    ...
    glutAddSubMenu("Change Parameters", parameters_sub_id);
...
void processInputs(int argc, char *argv[]) {
    ...
    if(argc > 4) {
        ...
        if(argc >= 3) {
            ...
            euler = false;
            inputFile = "test/flag.test";
            shapeFile = "shapes/empty.test";
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    scene = new Scene(inputFile, shapeFile, euler, useFloor);
